...
from pixivlib import PixivAPI, Formatter, PDFMaker
...
artworks = pxv.get_all_artworks(102881545)


artworks_data = {"artwork_id": [], "title": []}

for artwork_id, title in artworks.items():
    artworks_data["artwork_id"].append(artwork_id)
    artworks_data["title"].append(title)

artworks_data = (
    pl.DataFrame(artworks_data)
    .with_columns(pl.col("title").map_elements(fmt.zip_formatted))
    .sort("artwork_id")
)


title, illust_urls = pxv.get_illust_urls(119096047, init_access=False)

title = fmt.zip_formatted(title)
title = fmt.renamed(title, "./output")
"""